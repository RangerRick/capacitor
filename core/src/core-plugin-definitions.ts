...
export interface StoragePlugin extends Plugin {
   ...
   */
  get(options: { key: string, suiteName?: string }): Promise<{ value: string | null }>;
  /**
   * Set the value for the given key
   */
  set(options: { key: string, value: string, suiteName?: string }): Promise<void>;
  /**
   * Remove the value for this key (if any)
   */
  remove(options: { key: string, suiteName?: string }): Promise<void>;
  /**
   * Clear stored keys and values.
   */
  clear(options?: { suiteName?: string }): Promise<void>;
  /**
   * Return the list of known keys
   */
  keys(options?: { suiteName?: string }): Promise<{ keys: string[] }>;
}
